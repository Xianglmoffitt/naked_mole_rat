...
raw_count <- read.table("results/raw_count.txt",sep="\t",header=T)
...
sample_data <- data.frame(row.names = colnames(raw_count),
                          genotype = factor(c("WT","WT","ALK","ALK"),
                                            levels=c("WT","ALK")))
...
#===============================================================
# 1. gene expression changes between the WT and EML4-ALK+ cells
#===============================================================
...
# lfc shrinking
lfcs_res<- lfcShrink(dds,coef="genotype_ALK_vs_WT", res=res)
...
# PCA
pdf("results/PCA.pdf", width = 8,height = 6)
...
# correlation
pdf("results/correlation.pdf", width = 8,height = 4)
par(mfrow=c(1,2),cex.axis=1.2,cex.lab=1.2,font.lab=1.2)
for(i in 1:2){
    for(j in 1:1){
        x <- (i-1)*2+j
...
pdf("results/hist.pdf", width = 8,height = 8)
...
pdf("results/MA.pdf", width = 4,height = 4)
...
write.table(dat,file='results/result_normalized.txt',sep='\t',row.names=F,quote=F)
...
dat <- read.table("results/result_normalized.txt",sep='\t',header=T)
